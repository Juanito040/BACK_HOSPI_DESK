...
export class Ticket {
  ...
  toJSON() {
    ...
    // Include Prisma relations if available
    const prismaData = (this as any)._prismaData;
    if (prismaData) {
      if (prismaData.requester) {
        json.requester = {
          id: prismaData.requester.id,
          name: prismaData.requester.name,
          email: prismaData.requester.email,
          role: prismaData.requester.role,
        };
      }
      if (prismaData.assignedTo) {
        json.assignedTo = {
          id: prismaData.assignedTo.id,
          name: prismaData.assignedTo.name,
          email: prismaData.assignedTo.email,
          role: prismaData.assignedTo.role,
        };
      }
      if (prismaData.area) {
        json.area = {
          id: prismaData.area.id,
          name: prismaData.area.name,
          description: prismaData.area.description,
        };
      }
    }
